(function initLeadIntelligence(root, factory) {
  ...
  function scoreLead(input = {}) {
    const strategyId = input.strategyId || input.acquisitionStrategy || "uneed_presence";
    ...
    return { score: Math.min(100, Object.values(breakdown).reduce((sum, value) => sum + value, 0)), breakdown, version: POLICY_VERSION, strategyId };
  }
  ...
  function buildLeadIntelligence(input = {}) { const strategyId = input.strategyId || input.acquisitionStrategy || "uneed_presence"; const scoring = scoreLead({ ...input, strategyId }); const channels = selectChannels(input); const service = strategyId === "uneed_presence" ? recommendService(input) : null; const assessment = commercialAssessment(input); const messages = strategyId === "uneed_presence" ? followUps(input) : {}; const mockupData = strategyId === "uneed_presence" ? mockup(input, service) : { mockupRecommended: false, mockupReason: null, mockupPrompt: null }; const classified = classifyUrl(input.website); const confidence = input.confidence === "high" || input.confidence === "medium" || input.confidence === "low" ? input.confidence : Number(input.confidence || 0) >= .8 ? "high" : Number(input.confidence || 0) >= .55 ? "medium" : "low"; const confidenceReason = [classified.official ? "website oficial confirmado" : "website oficial não confirmado", input.instagramUrl ? "Instagram confirmado" : "perfil social não confirmado", input.phone || input.email ? "canal direto disponível" : "canal direto não confirmado"];
    return { schemaVersion: POLICY_VERSION, strategyId, acquisitionStrategy: strategyId, qualificationProfileVersion: strategyId === "uneed_presence" ? "uneed-presence-v0.1" : "not_configured", messagePolicyVersion: strategyId === "uneed_presence" ? "uneed-presence-outreach-v0.1" : "not_configured", commercialScore: scoring.score, scoreBreakdown: scoring.breakdown, scoreVersion: scoring.version, confidence, confidenceReason, whyThisLead: assessment.whyContactThisLead, whyContactThisLead: assessment.whyContactThisLead, mainOpportunity: assessment.improvementOpportunities[0].label, mainCommercialGap: assessment.mainCommercialGap, confirmedGaps: assessment.confirmedGaps, improvementOpportunities: assessment.improvementOpportunities, unknowns: assessment.unknowns, communicationQuality: assessment.communicationQuality, serviceId: service?.serviceId || null, recommendedService: service?.name || null, uneedFit: service ? { serviceId: service.serviceId, fit: "high", reason: ["negócio local", "necessidade de presença profissional ou marcações", "solução mensal sem fidelização"] } : { serviceId: null, fit: "review", reason: ["estratégia ainda sem inteligência comercial configurada"] }, serviceDetails: service || null, ...channels, initialMessage: strategyId === "uneed_presence" ? initialMessage(input, service) : null, ...messages, ...mockupData, officialWebsite: classified.official ? classified.url : null, externalReferences: classified.official || !classified.url ? [] : [{ url: classified.url, type: classified.type }], readiness: "researching", createdBy: "automated_lead_factory", createdAt: new Date().toISOString(), lastResearchedAt: null, nextAction: "Aguardar conclusão da investigação" }; }
