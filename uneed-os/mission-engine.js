...
const { getStrategy } = require("../acquisition-strategies");
...
class MissionEngine {
  ...
  async create({ type = "qualify_existing_lead", objective, targetType = "lead", targetId, requestedBy, autonomyLevel = "PREPARE", maxCost = 1, maxAttempts = 3, input = {}, idempotencyKey }) {
    ...
    const requestedStrategy = input.acquisitionStrategy || input.lead?.acquisitionStrategy || null; if (input.leadFactoryBatchId && !getStrategy(requestedStrategy)) throw new RuntimeError("INPUT_INVALID", "Mission Lead Factory sem acquisitionStrategy válida."); const duplicate = await this.repository.findActiveDuplicate(requestedBy, type, targetType, targetId); const duplicateStrategy = duplicate?.input?.acquisitionStrategy || duplicate?.input?.lead?.acquisitionStrategy || null; const duplicateMatchesStrategy = !requestedStrategy || (duplicate?.metadata?.strategyAware === true && duplicate.metadata.acquisitionStrategy === requestedStrategy && duplicateStrategy === requestedStrategy); if (duplicate && duplicateMatchesStrategy) return { mission: duplicate, duplicate: true };
    ...
    const mission = { id: crypto.randomUUID(), type, objective, status: "draft", priority: "normal", targetType, targetId, requestedBy, autonomyLevel, strategy, steps, currentStepCount: 0, maxSteps: Math.min(Number(this.maxSteps), 100), input, result: null, confidence: null, maxCost: Number(maxCost), estimatedCost: 0, actualCost: 0, costStatus: "actual", callCount: 0, inputTokens: 0, outputTokens: 0, attempt: 0, maxAttempts: Math.min(Math.max(Number(maxAttempts), 1), 5), nextRunAt: null, workerId: null, claimedAt: null, heartbeatAt: null, cancelRequestedAt: null, startedAt: null, completedAt: null, createdAt, updatedAt: createdAt, error: null, lastError: null, metadata: { acquisitionStrategy: requestedStrategy, strategyAware: Boolean(requestedStrategy) }, events: [event("mission.created", "Missão criada", { acquisitionStrategy: requestedStrategy }), event("mission.queued", "Missão colocada em fila")] };
  ...
  async executeSkillStep(mission, step, workerId) {
    ...
    const company = mission.input.company || null; const contact = mission.input.contact || null; const centralLead = mission.input.lead || {}; const explicitStrategy = mission.input.acquisitionStrategy || mission.metadata?.acquisitionStrategy || null; if (mission.metadata?.strategyAware && (!explicitStrategy || explicitStrategy !== mission.metadata.acquisitionStrategy || !getStrategy(explicitStrategy))) throw new RuntimeError("INPUT_INVALID", "acquisitionStrategy da Mission ausente ou inconsistente."); const context = { company, contact, acquisitionStrategy: explicitStrategy || centralLead.acquisitionStrategy || null, serviceId: explicitStrategy === "high_ticket" ? null : mission.input.serviceId || centralLead.serviceId || null, intelligenceSeed: mission.input.intelligenceSeed || null, lead: company ? { ...centralLead, companyName: company.name, companyWebsite: company.website, clientName: contact?.name || "", clientEmail: contact?.email || company.email || "", clientPhone: contact?.phone || company.phone || "", instagram: company.instagramUrl || "", location: company.location || "", internalNotes: centralLead.notes || company.notes || "", services: explicitStrategy === "high_ticket" ? [] : mission.input.services || [] } : centralLead }; for (const previous of mission.steps.filter((item) => item.output && item.skillId)) context[resultKey(previous.skillId)] = previous.output;
    ...
    if (!validateSkillOutput(skill.id, response.output, skill.version)) throw new RuntimeError("AI_BAD_RESPONSE", `Output inválido para ${skill.id}@${skill.version}.`); if (context.acquisitionStrategy === "high_ticket") { const serialized = JSON.stringify(response.output); const forbidden = [/uneed_presence/i,/Uneed Presença/i,/39\s*€/i,/sem fidelização/i,/simulação personalizada/i,/pedidos de marcação integrados/i,/ready_for_contact/i]; if (forbidden.some((pattern) => pattern.test(serialized))) throw new RuntimeError("AI_BAD_RESPONSE", `Output High Ticket contaminado por policy Presença em ${skill.id}.`); if (skill.id === "qualify-lead" && (response.output.qualificationProfile !== "high_ticket_v1" || response.output.serviceId !== null)) throw new RuntimeError("AI_BAD_RESPONSE", "Qualificação High Ticket com profile ou service_id inválido."); if (skill.id === "prepare-outreach" && response.output.requiredHumanReview !== "strict") throw new RuntimeError("AI_BAD_RESPONSE", "Outreach High Ticket sem revisão estrita."); }
