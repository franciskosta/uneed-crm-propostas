...
class MissionEngine {
  ...
  async executeSkillStep(mission, step, workerId) {
    ...
    const context = { lead: mission.input.lead }; for (const previous of mission.steps.filter((item) => item.output && item.skillId)) context[resultKey(previous.skillId)] = previous.output;
    ...
    if (skill.id === "research-company" && skill.version === "1.1.0") {
      ...
      const discovery = await this.discover.research(mission.input.lead, { budget: remainingBudget }); context.researchPack = discovery.pack; mission.toolEstimatedCost = Number(mission.toolEstimatedCost || 0) + Number(discovery.toolEstimatedCost || 0); if (discovery.toolCost == null) mission.toolCostStatus = "unknown"; else mission.toolCost = Number(mission.toolCost || 0) + discovery.toolCost;
    ...
    const response = await this.gateway.execute({ task: skill.id, capabilitiesRequired: skill.requiredCapabilities, quality: skill.id === "qualify-lead" ? "SMART" : "FAST", budget: remainingBudget, allowFallback: true, context, instructions: skill.instructions, outputSchema: { type: "object", required: OUTPUT_FIELDS[skill.id] } });
