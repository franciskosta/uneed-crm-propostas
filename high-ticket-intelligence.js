(function initHighTicket(root, factory) {
  ...
  function hypotheses(pack = {}) {
    ...
    if (s.catalogue?.length || s.b2b?.length) add("sales_process", "Validar a integração entre catálogo, pedidos comerciais e acompanhamento de clientes.", "A oferta ou o modelo comercial observado pode implicar vários pontos de passagem de informação.", "Menos duplicação e maior visibilidade do processo comercial.", ["catálogo","produto","b2b","orçamento","distribui"]);
    if (s.locations?.length) add("multi_location", "Validar como informação e processos são coordenados entre localizações.", "Foram observados sinais públicos de operação em mais do que uma localização.", "Maior consistência operacional e reporting consolidado.", ["localiza","loja","delegação","rede"]);
    if (s.operations?.length || s.manual?.length) add("internal_workflow", "Validar os fluxos administrativos e operacionais com maior carga manual.", "Existem sinais públicos de processos, documentos, suporte ou logística que justificam diagnóstico.", "Redução de tarefas repetitivas, erros e atrasos.", ["logística","stock","document","formulário","suporte","pedido"]);
    if (s.growth?.length) add("reporting", "Validar se o crescimento é acompanhado por reporting e sistemas adequados.", "Recrutamento, expansão ou exportação podem aumentar a necessidade de coordenação.", "Melhor visibilidade de gestão e capacidade de escalar.", ["carreira","recrut","expans","export"]);
  ...
  function buildOutreach(input, pack, opportunities) {
    ...
    const observation = first.hypothesis.replace(/^Validar /i, "a forma como ").replace(/\.$/, "");
    return { preparedMessage: `Bom dia, sou o Francisco, da Uneed Soluções Digitais. Estive a conhecer melhor a ${company} e chamou-me a atenção ${observation}. Há uma hipótese de a tecnologia simplificar trabalho ou melhorar o acompanhamento, mas só faz sentido validá-la com quem conhece a operação. Temos um Diagnóstico Digital para identificar oportunidades antes de falar em qualquer solução. Se não encontrarmos impacto real, ficamos por aí. Posso enviar-lhe o acesso?`, readiness: "ready_for_review", reason: "Mensagem personalizada a partir de evidência pública; exige revisão humana estrita." };
  }
  function buildHighTicketIntelligence(input = {}, pack = {}) {
    ...
    const confirmedSignals = Object.entries(pack.highTicketSignals || {}).flatMap(([category, items]) => (items || []).map((item) => ({ category, statement: item.statement, evidenceIds: item.evidenceIds || [] })));
