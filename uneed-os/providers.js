const { scoreLead, recommendService, buildLeadIntelligence } = require("../lead-intelligence");
  ...
  async execute(request) {
    ...
    if (request.task === "qualify-lead") {
      if (request.context.research?.schemaVersion) {
        const pack = request.context.research; const signals = pack.websiteSignals || {}; const catalog = pack.knownData?.catalog || []; const scoring = scoreLead({ website: pack.digitalPresence?.website, instagramUrl: pack.digitalPresence?.socialProfiles?.find((item) => item.platform === "instagram")?.url, phone: pack.knownData?.contacts?.phone, email: pack.knownData?.contacts?.email, niche: lead.niche || lead.opportunityCategory, municipality: lead.municipality, district: lead.district, hasBooking: signals.booking, hasCta: Boolean(signals.contactPaths?.length) }); const score = scoring.score; const service = recommendService({ website: pack.digitalPresence?.website, hasBooking: signals.booking, hasCta: Boolean(signals.contactPaths?.length) }, catalog); const confidence = pack.confidence === "high" ? 0.85 : pack.confidence === "medium" ? 0.65 : 0.4;
        return { output: { score, breakdown: scoring.breakdown, scoreVersion: scoring.version, opportunityLevel: score >= 70 ? "alta" : score >= 40 ? "média" : "por qualificar", observations: pack.observations || [], digitalPresence: pack.digitalPresence, problemsDetected: pack.inferences || [], potentialNeeds: pack.opportunities || [], recommendedService: service, reasoningSummary: `Score comercial ${scoring.version}, calculado apenas com sinais explicáveis do Lead e Research Pack.`, confidence }, cost: 0, usage: { calls: 1 } };
      }
    ...
    if (request.task === "prepare-outreach") {
      ...
      if (request.context.intelligenceSeed) { const intelligence = buildLeadIntelligence({ ...request.context.intelligenceSeed, website: request.context.research?.digitalPresence?.website || request.context.intelligenceSeed.website }, request.context.research?.knownData?.catalog || []); return { output: { suggestedApproach: `Contacto manual por ${intelligence.primaryChannel}${intelligence.fallbackChannel ? `; alternativa ${intelligence.fallbackChannel}` : ""}.`, preparedMessage: intelligence.initialMessage, followUp1: intelligence.followUp1, followUp2: intelligence.followUp2, recommendedFollowUpDelayDays: intelligence.recommendedFollowUpDelayDays, recommendationReason: qualification.reasoningSummary || intelligence.whyThisLead, evidenceUsed: request.context.research?.facts?.flatMap((item) => item.evidenceIds || []).slice(0, 5) || [] }, cost: 0, usage: { calls: 0 } }; }
