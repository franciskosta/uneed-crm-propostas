...
function renderMissionResult(mission, compact = false) {
  ...
  return `<article class="mission-result ${compact ? "compact" : ""}">
    ...
    ${mission.result ? `<div class="mission-score"><div><span>${qualification.score == null ? "Confiança da investigação" : "Opportunity Score"}</span><strong>${qualification.score == null ? formatConfidence(research.confidence) : `${escapeHtml(qualification.score)}/100`}</strong></div>${qualification.score != null ? `<div><span>Confiança</span><strong>${formatConfidence(qualification.confidenceLevel || mission.confidence)}</strong><small>${escapeHtml((qualification.confidenceReason || []).join(" · "))}</small></div>` : ""}</div>
...
function reconcileLeadFactoryMissions() {
  ...
  for (const prospect of state.instagramProspects || []) {
    ...
    prospect.researchPack = research; prospect.commercialIntelligence = { qualification, outreach }; prospect.lastResearchedAt = research.researchedAt || mission.updatedAt; prospect.score = prospect.commercialScore = Number(qualification.score ?? prospect.commercialScore ?? prospect.score ?? 0); prospect.scoreBreakdown = qualification.breakdown || prospect.scoreBreakdown; prospect.scoreVersion = qualification.scoreVersion || prospect.scoreVersion; prospect.confidence = qualification.confidenceLevel || (qualification.confidence ?? prospect.confidence); prospect.confidenceReason = qualification.confidenceReason || prospect.confidenceReason; prospect.whyContactThisLead = qualification.whyContactThisLead || prospect.whyContactThisLead; prospect.mainCommercialGap = qualification.mainCommercialGap || prospect.mainCommercialGap; prospect.opportunity = structuredText(qualification.improvementOpportunities?.[0] || qualification.potentialNeeds?.[0]) || prospect.mainOpportunity || prospect.opportunity; prospect.mainOpportunity = prospect.opportunity; prospect.confirmedGaps = qualification.confirmedGaps || qualification.problemsDetected || []; prospect.improvementOpportunities = qualification.improvementOpportunities || qualification.potentialNeeds || []; prospect.communicationQuality = qualification.communicationQuality || null; prospect.serviceId = qualification.serviceId || prospect.serviceId; prospect.recommendedService = qualification.recommendedService || prospect.recommendedService; prospect.serviceDetails = qualification.serviceDetails || prospect.serviceDetails; prospect.uneedFit = qualification.uneedFit || prospect.uneedFit; prospect.message = outreach.preparedMessage || prospect.initialMessage || prospect.message; prospect.initialMessage = prospect.message; prospect.followUp1 = outreach.followUp1 || prospect.followUp1; prospect.followUp2 = outreach.followUp2 || prospect.followUp2; prospect.recommendedFollowUpDelayDays = outreach.recommendedFollowUpDelayDays || prospect.recommendedFollowUpDelayDays; prospect.officialWebsite = research.digitalPresence?.website || prospect.officialWebsite || null; if (prospect.officialWebsite) { prospect.website = prospect.officialWebsite; prospect.hasWebsite = true; } prospect.warnings = research.warnings || []; prospect.unknowns = [...new Set([...(qualification.unknowns || []), ...(research.unknowns || [])])]; prospect.sources = research.evidence || []; const threshold = Number(prospect.minimumScore || 0); const completeForContact = Boolean(prospect.name && (prospect.primaryChannel && prospect.primaryChannel !== "manual_review") && prospect.mainCommercialGap && prospect.serviceId && prospect.message); prospect.readiness = prospect.commercialScore >= threshold && completeForContact ? "ready_for_contact" : "needs_review"; prospect.nextAction = prospect.readiness === "ready_for_contact" ? `Contactar por ${prospect.primaryChannel}` : "Rever identidade, canal, oportunidade, solução ou mensagem"; prospect.updatedAt = new Date().toISOString();
