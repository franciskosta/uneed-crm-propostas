...
function renderStructuredList(items, emptyText = "Sem informação confirmada.") { const values = [...new Set((Array.isArray(items) ? items : items ? [items] : []).map(structuredText).filter(Boolean))]; return `<ul>${values.map((item) => `<li>${escapeHtml(item)}</li>`).join("") || `<li>${escapeHtml(emptyText)}</li>`}</ul>`; }
function renderHighTicketImpact(items) { const values = [...new Set((items || []).map((item) => structuredText(item?.impact || item)).filter(Boolean))]; return renderStructuredList(values, "Ainda por validar."); }
function renderPresenceIntelligence(qualification, outreach) { return `<section class="mission-commercial-summary"><h4>Porque contactar</h4><p>${escapeHtml(qualification.whyContactThisLead || qualification.reasoningSummary || "A validar")}</p><h4>Principal oportunidade</h4><p>${escapeHtml(qualification.mainCommercialGap || structuredText(qualification.potentialNeeds?.[0]) || "A validar")}</p><h4>Solução UNEED</h4><p><strong>${escapeHtml(qualification.recommendedService || "A validar")}</strong>${qualification.serviceDetails?.priceFrom ? ` · desde ${escapeHtml(qualification.serviceDetails.priceFrom)}€ + IVA/mês · sem fidelização` : ""}</p><h4>Estratégia</h4><p>${escapeHtml(outreach.suggestedApproach || "A validar")}</p><h4>Mensagem</h4><blockquote>${escapeHtml(outreach.preparedMessage || "A preparar")}</blockquote><h4>Canais</h4><p>${escapeHtml(outreach.suggestedApproach?.split(". Ângulo:")[0] || "A validar")}</p></section><h4>Lacunas confirmadas</h4>${renderStructuredList(qualification.confirmedGaps || qualification.problemsDetected, "Nenhuma lacuna confirmada com a evidência disponível.")}<h4>Oportunidades de melhoria</h4>${renderStructuredList(qualification.improvementOpportunities || qualification.potentialNeeds, "A validar comercialmente.")}<h4>Qualidade da comunicação digital</h4><p>${escapeHtml(qualification.communicationQuality ? `${structuredText(qualification.communicationQuality)} ` : "Ainda não avaliada.")}</p>`; }
function renderHighTicketIntelligence(qualification, outreach) { const signals = (qualification.confirmedOperationalSignals || []).map((item) => item?.statement || item); const hypotheses = (qualification.opportunityHypotheses || []).map((item) => item?.hypothesis || item); const makers = (qualification.decisionMakerSignals || []).map((item) => item?.name ? `${item.name} — ${item.role}` : item?.role || item); return `<section class="mission-commercial-summary high-ticket-intelligence"><h4>Porque merece investigação</h4><p>${escapeHtml(qualification.whyInvestigate || qualification.reasoningSummary || "Evidência insuficiente; aprofundar investigação.")}</p><h4>Sinais operacionais confirmados</h4>${renderStructuredList(signals, "Sem sinais suficientes.")}<h4>Hipóteses de oportunidade</h4>${renderStructuredList(hypotheses, "A investigação precisa de ser aprofundada.")}<h4>Potencial de impacto</h4>${renderHighTicketImpact(qualification.businessImpactHypotheses)}<h4>Perguntas para validar</h4>${renderStructuredList(qualification.discoveryQuestions)}<h4>Decisor provável</h4>${renderStructuredList(makers, "Desconhecido; não bloqueia o Lead.")}<h4>Próximo passo</h4><p><a href="https://diagnostico-digital.uneed.pt/" target="_blank" rel="noopener noreferrer">Diagnóstico Digital Uneed</a></p><h4>Mensagem High Ticket</h4><blockquote>${escapeHtml(outreach.preparedMessage || "Ainda sem evidência suficiente para preparar uma mensagem.")}</blockquote></section>`; }
...
function renderMissionResult(mission, compact = false) {
  ...
  const highTicket = qualification.strategyId === "high_ticket" || mission.input?.acquisitionStrategy === "high_ticket";
  ...
  return `<article class="mission-result ${compact ? "compact" : ""}">
    ...
    ${mission.result ? `<div class="mission-score"><div><span>${qualification.score == null ? "Confiança da investigação" : highTicket ? "High Ticket Fit" : "Opportunity Score"}</span><strong>${qualification.score == null ? formatConfidence(research.confidence) : `${escapeHtml(qualification.score)}/100`}</strong></div>${qualification.score != null ? `<div><span>Confiança</span><strong>${formatConfidence(qualification.confidenceLevel || qualification.confidence || mission.confidence)}</strong><small>${escapeHtml(Array.isArray(qualification.confidenceReason) ? qualification.confidenceReason.join(" · ") : qualification.confidenceReason || "")}</small></div>` : ""}</div>
    ${highTicket ? renderHighTicketIntelligence(qualification, outreach) : renderPresenceIntelligence(qualification, outreach)}
    <h4>Business Summary</h4><p>${escapeHtml(research.businessSummary || "Sem resumo")}</p>
    <h4>Evidência confirmada</h4>${renderStructuredList(research.facts, "Sem factos adicionais.")}
    ...
    <h4>Observações</h4>${renderStructuredList(research.observations, "Sem observações adicionais.")}
    <h4>Não confirmado</h4>${renderStructuredList([...(qualification.unknowns || []), ...(research.unknowns || [])], "Sem lacunas de informação registadas.")}
...
function appendFactoryActivity(prospect, mission, summary) {
  ...
  state.activities.push({ id: crypto.randomUUID(), companyId: prospect.companyId || null, leadId: prospect.leadId || null, opportunityId: null, proposalId: null, type: "lead_factory", summary, channel: prospect.primaryChannel || null, acquisitionStrategy: prospect.acquisitionStrategy || null, serviceId: prospect.serviceId || null, occurredAt: mission.updatedAt || new Date().toISOString(), legacyRef, createdAt: new Date().toISOString() });
}
...
function reconcileLeadFactoryMissions() {
  ...
  for (const prospect of state.instagramProspects || []) {
    ...
    if (!mission) continue;
    if (prospect.acquisitionStrategy === "high_ticket" && isHighTicketContaminated(mission)) { const warning = "Análise anterior ignorada: continha lógica de Presença. Executa novamente a investigação High Ticket."; if (prospect.readiness !== "needs_more_research" || !(prospect.warnings || []).includes(warning)) { prospect.readiness = "needs_more_research"; prospect.serviceId = null; prospect.recommendedService = null; prospect.serviceDetails = null; prospect.uneedFit = null; prospect.followUp1 = null; prospect.followUp2 = null; prospect.mockupImage = null; prospect.message = ""; prospect.warnings = [...new Set([...(prospect.warnings || []), warning])]; changed = true; } continue; }
    ...
    prospect.researchHistory.push({ missionId: mission.id, strategyId: prospect.acquisitionStrategy || mission.input?.acquisitionStrategy || null, researchedAt: research.researchedAt || mission.updatedAt, researchPack: research, commercialIntelligence: { qualification, outreach }, costs: { actual: mission.actualCost ?? null, estimated: mission.estimatedCost ?? null, toolActual: mission.toolCost ?? null, toolEstimated: mission.toolEstimatedCost ?? null } });
    prospect.researchPack = research; prospect.commercialIntelligence = { qualification, outreach }; prospect.lastResearchedAt = research.researchedAt || mission.updatedAt; prospect.score = prospect.commercialScore = Number(qualification.score ?? prospect.commercialScore ?? prospect.score ?? 0); prospect.highTicketFit = qualification.highTicketFit ?? prospect.highTicketFit; prospect.scoreBreakdown = qualification.breakdown || prospect.scoreBreakdown; prospect.scoreVersion = qualification.scoreVersion || prospect.scoreVersion; prospect.confidence = qualification.confidenceLevel || (qualification.confidence ?? prospect.confidence); prospect.confidenceReason = qualification.confidenceReason || prospect.confidenceReason; prospect.whyContactThisLead = qualification.whyContactThisLead || qualification.reasoningSummary || prospect.whyContactThisLead; prospect.mainCommercialGap = qualification.mainCommercialGap || structuredText(qualification.opportunityHypotheses?.[0]) || prospect.mainCommercialGap; prospect.opportunity = structuredText(qualification.opportunityHypotheses?.[0] || qualification.improvementOpportunities?.[0] || qualification.potentialNeeds?.[0]) || prospect.mainOpportunity || prospect.opportunity; prospect.mainOpportunity = prospect.opportunity; prospect.confirmedGaps = qualification.confirmedOperationalSignals || qualification.confirmedGaps || qualification.problemsDetected || []; prospect.improvementOpportunities = qualification.opportunityHypotheses || qualification.improvementOpportunities || qualification.potentialNeeds || []; prospect.discoveryQuestions = qualification.discoveryQuestions || []; prospect.decisionMakerSignals = qualification.decisionMakerSignals || []; prospect.recommendedNextStep = qualification.recommendedNextStep || null; prospect.communicationQuality = qualification.communicationQuality || null; prospect.serviceId = qualification.serviceId || prospect.serviceId; prospect.recommendedService = qualification.recommendedService || prospect.recommendedService; prospect.serviceDetails = qualification.serviceDetails || prospect.serviceDetails; prospect.uneedFit = qualification.uneedFit || prospect.uneedFit; prospect.message = outreach.preparedMessage || prospect.initialMessage || prospect.message; prospect.initialMessage = prospect.message; prospect.followUp1 = outreach.followUp1 || prospect.followUp1; prospect.followUp2 = outreach.followUp2 || prospect.followUp2; prospect.recommendedFollowUpDelayDays = outreach.recommendedFollowUpDelayDays || prospect.recommendedFollowUpDelayDays; prospect.officialWebsite = research.digitalPresence?.website || prospect.officialWebsite || null; if (prospect.officialWebsite) { prospect.website = prospect.officialWebsite; prospect.hasWebsite = true; } prospect.warnings = research.warnings || []; prospect.unknowns = [...new Set([...(qualification.unknowns || []), ...(research.unknowns || [])])]; prospect.sources = research.evidence || []; const threshold = Number(prospect.minimumScore || 0); const highTicket = prospect.acquisitionStrategy === "high_ticket"; const completeForContact = Boolean(prospect.name && (prospect.primaryChannel && prospect.primaryChannel !== "manual_review") && prospect.mainCommercialGap && prospect.serviceId && prospect.message); prospect.readiness = highTicket ? (qualification.readiness || "needs_more_research") : prospect.commercialScore >= threshold && completeForContact ? "ready_for_contact" : "needs_review"; prospect.nextAction = highTicket ? (prospect.readiness === "ready_for_review" ? "Rever e decidir convite para Diagnóstico Digital" : prospect.readiness === "reject" ? "Arquivar ou rejeitar" : "Aprofundar investigação") : prospect.readiness === "ready_for_contact" ? `Contactar por ${prospect.primaryChannel}` : "Rever identidade, canal, oportunidade, solução ou mensagem"; prospect.updatedAt = new Date().toISOString();
    if (prospect.acquisitionStrategy === "high_ticket") { prospect.serviceId = null; prospect.recommendedService = null; prospect.serviceDetails = null; prospect.uneedFit = null; prospect.followUp1 = null; prospect.followUp2 = null; prospect.mockupImage = null; if (prospect.readiness === "ready_for_review" && prospect.commercialScore < Number(prospect.minimumScore || 0)) { prospect.readiness = "reject"; prospect.nextAction = `Rejeitado: High Ticket Fit ${prospect.commercialScore} abaixo do mínimo ${prospect.minimumScore}`; } }
...
async function startLeadMission(type = "qualify_existing_lead") {
  ...
  await createCommercialMission({ type, objective: `${type === "research_company" ? "Investigar" : "Qualificar"} ${context.company.name}`, targetId: type === "research_company" ? context.company.id : context.lead.id, targetType: type === "research_company" ? "company" : "lead", input: { ...context, acquisitionStrategy: context.lead?.acquisitionStrategy || "uneed_presence", serviceId: context.lead?.serviceId || "uneed_presence", services: (proposal.services || []).filter((item) => item.selected !== false).map((item) => ({ name: item.name, selected: true })) }, trigger });
}
...
function createMonochromeLogoCanvas(logoImage, maxWidth, maxHeight) {
  ...
  const cornerSize = Math.max(1, Math.round(Math.min(sourceWidth, sourceHeight) * 0.04));
  const edgeColor = { red: 0, green: 0, blue: 0, alpha: 0, count: 0 };
  const sampleCorner = (startX, startY) => {
    for (let y = startY; y < startY + cornerSize; y += 1) {
      for (let x = startX; x < startX + cornerSize; x += 1) {
        const offset = (y * sourceWidth + x) * 4;
        edgeColor.red += sourcePixels[offset];
        edgeColor.green += sourcePixels[offset + 1];
        edgeColor.blue += sourcePixels[offset + 2];
        edgeColor.alpha += sourcePixels[offset + 3];
        edgeColor.count += 1;
      }
    }
  };
  sampleCorner(0, 0);
  sampleCorner(sourceWidth - cornerSize, 0);
  sampleCorner(0, sourceHeight - cornerSize);
  sampleCorner(sourceWidth - cornerSize, sourceHeight - cornerSize);
  edgeColor.red /= edgeColor.count;
  edgeColor.green /= edgeColor.count;
  edgeColor.blue /= edgeColor.count;
  edgeColor.alpha /= edgeColor.count;
  const edgeLuminance = edgeColor.red * 0.299 + edgeColor.green * 0.587 + edgeColor.blue * 0.114;
  const isBackgroundCandidate = (red, green, blue, alpha) => {
    if (alpha <= 16) return true;
    const luminance = red * 0.299 + green * 0.587 + blue * 0.114;
    const chroma = Math.max(red, green, blue) - Math.min(red, green, blue);
    const edgeDistance = Math.hypot(red - edgeColor.red, green - edgeColor.green, blue - edgeColor.blue);
    const isFlatLight = chroma < 30 && luminance > 235;
    const isFlatDark = chroma < 24 && luminance < 42;
    const edgeTolerance = edgeLuminance < 55 || edgeLuminance > 220 ? 52 : 38;
    return isFlatLight || isFlatDark || (edgeColor.alpha > 180 && edgeDistance < edgeTolerance);
  };
  ...
  const enqueueBackground = (x, y) => {
    const point = y * sourceWidth + x;
    if (queued[point]) return;
    const offset = point * 4;
    if (!isBackgroundCandidate(sourcePixels[offset], sourcePixels[offset + 1], sourcePixels[offset + 2], sourcePixels[offset + 3])) return;
    queued[point] = 1;
  ...
  while (queueStart < queueEnd) {
    ...
    queueStart += 1;
    backgroundMask[point] = 1;
    const x = point % sourceWidth;
    const y = Math.floor(point / sourceWidth);
    if (x > 0) enqueueBackground(x - 1, y);
    if (x + 1 < sourceWidth) enqueueBackground(x + 1, y);
    if (y > 0) enqueueBackground(x, y - 1);
    if (y + 1 < sourceHeight) enqueueBackground(x, y + 1);
  }
  ...
  for (let y = 0; y < sourceHeight; y += 1) {
    for (let x = 0; x < sourceWidth; x += 1) {
      ...
      if (!backgroundMask[y * sourceWidth + x] && sourcePixels[index + 3] > 16) {
  ...
  const boundsPadding = Math.max(2, Math.round(Math.min(right - left + 1, bottom - top + 1) * 0.025));
  left = Math.max(0, left - boundsPadding);
  top = Math.max(0, top - boundsPadding);
  right = Math.min(sourceWidth - 1, right + boundsPadding);
  bottom = Math.min(sourceHeight - 1, bottom + boundsPadding);
  ...
  for (let index = 0; index < pixels.length; index += 4) {
    ...
    pixels[index + 1] = 255;
    pixels[index + 2] = 255;
    const sourceX = left + ((index / 4) % cropWidth);
    const sourceY = top + Math.floor(index / 4 / cropWidth);
    pixels[index + 3] = backgroundMask[sourceY * sourceWidth + sourceX] || alpha <= 16 ? 0 : Math.max(alpha, 218);
  }
...
async function createProspectMockupImage(prospect, logoDataUrl, fields) {
  ...
  drawTemplateLogo(ctx, logoImage, name, 150, 114, 235, 64, { fontSize: 23 });
  drawTemplateLogo(ctx, logoImage, name, 1213, 268, 140, 46, { fontSize: 14 });
...
async function generateProspects() {
  ...
  try {
    ...
    for (const lead of payload.results || []) {
      ...
      added.push({ ...emptyInstagramProspect(), ...lead, message: formalizeProspectMessage(lead.message), id: crypto.randomUUID(), status: "Por fazer", source: "automated_lead_factory", discoverySource: lead.discoverySource || "google_places", leadFactoryBatchId: batchId, acquisitionStrategy, strategyId: acquisitionStrategy, serviceId: lead.serviceId || payload.serviceId || null, questEligible: window.UNEED_ACQUISITION_STRATEGIES?.getStrategy(acquisitionStrategy)?.questEligible === true, minimumScore, readiness: "researching", researchHistory: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
    }
              ...
              .map((prospect) => {
                ...
                return `
                  <article class="deal-card prospect-card" data-instagram-open="${escapeAttr(prospect.id)}" draggable="true">
                    ...
                    <span class="prospect-readiness readiness-${escapeAttr(prospect.readiness || "not_researched")}">${escapeHtml((prospect.readiness || "not_researched").replaceAll("_", " "))}</span>${prospect.acquisitionStrategy ? `<span class="prospect-strategy">${escapeHtml(prospect.acquisitionStrategy === "uneed_presence" ? "PRESENÇA" : "HIGH TICKET")}</span>` : ""}
                    ${prospect.score ? `<span class="prospect-score-line">Score ${escapeHtml(prospect.score)} · Confiança ${escapeHtml(formatConfidence(prospect.confidence))} · ${escapeHtml(prospect.district || "")} / ${escapeHtml(prospect.municipality || "")}</span>` : ""}
                    ${prospect.whyContactThisLead || prospect.whyThisLead ? `<span class="card-meta"><strong>Porque contactar:</strong> ${escapeHtml(prospect.whyContactThisLead || prospect.whyThisLead)}</span>` : ""}
                    ${prospect.mainCommercialGap || prospect.mainOpportunity ? `<span class="card-meta"><strong>Principal oportunidade:</strong> ${escapeHtml(prospect.mainCommercialGap || prospect.mainOpportunity)}</span>` : ""}
...
function contractClauseBlocks(contract) {
  ...
  blocks.push(["Domínio, email e alojamento", "Nos planos que incluam domínio, alojamento ou email profissional, estes serviços são assegurados enquanto o contrato estiver ativo e os pagamentos estiverem regularizados. O alojamento e o email não continuam automaticamente após o cancelamento do contrato.\n\nNo final do serviço, o Cliente poderá solicitar a transferência do domínio, desde que os pagamentos estejam totalmente regularizados, a transferência seja tecnicamente possível e sejam cumpridos os procedimentos e prazos do respetivo fornecedor ou registador. Acedendo a estas condições, a disponibilização do código de transferência (Auth-Code) será efetuada sem qualquer custo adicional.\n\nSempre que necessário, a Uneed poderá conceder um período máximo razoável de 15 (quinze) dias para a migração do domínio ou exportação de dados, incluindo cópia de segurança do site e emails, por parte do Cliente, antes da desativação definitiva dos serviços e eliminação permanente dos dados dos servidores.\n\nCaso o Cliente solicite assistência técnica direta à Uneed para a realização de migrações de ficheiros, bases de dados ou histórico de emails para servidores de terceiros, este serviço será orçamentado autonomamente de acordo com a tarifa horária de suporte em vigor."]);
