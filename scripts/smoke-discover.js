...
if (!name) { console.error("Uso: node scripts/smoke-discover.js 'Empresa' 'https://website.pt'"); process.exitCode = 1; }
  ...
  new DiscoverService({ searchProvider: provider }).research({ companyName: name, website, strategyId: process.env.DISCOVER_STRATEGY || "uneed_presence" }, { budget: Number(process.env.DISCOVER_SMOKE_MAX_COST || 1) }).then((result) => console.log(JSON.stringify(result.pack, null, 2))).catch((error) => { console.error(`${error.code || "ERROR"}: ${error.message}`); process.exitCode = 1; });
}
