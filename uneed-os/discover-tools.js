...
class ReadCrmTool extends Tool {
  ...
  async execute(input) { const lead = input?.lead || {}; return { data: { name: lead.companyName || lead.clientName || "", website: lead.website || lead.sampleUrl || "", location: lead.location || "", email: lead.clientEmail || "", phone: lead.clientPhone || "", instagram: lead.instagram || "", observations: lead.internalNotes || "", source: lead.leadSource || "", catalog: (lead.catalog || lead.services || []).filter((item) => item.selected !== false).map((item) => ({ name: item.name })).filter((item) => item.name) }, cost: 0, costStatus: "actual" }; }
}
