...
function renderStructuredList(items, emptyText = "Sem informação confirmada.") { const values = (Array.isArray(items) ? items : items ? [items] : []).map(structuredText).filter(Boolean); return `<ul>${values.map((item) => `<li>${escapeHtml(item)}</li>`).join("") || `<li>${escapeHtml(emptyText)}</li>`}</ul>`; }
...
function renderHighTicketIntelligence(qualification, outreach) { return `<section class="mission-commercial-summary high-ticket-intelligence"><h4>Porque merece investigação</h4><p>${escapeHtml(qualification.whyInvestigate || qualification.reasoningSummary || "Evidência insuficiente; aprofundar investigação.")}</p><h4>Sinais operacionais confirmados</h4>${renderStructuredList(qualification.confirmedOperationalSignals, "Sem sinais suficientes.")}<h4>Hipóteses de oportunidade</h4>${renderStructuredList(qualification.opportunityHypotheses, "A investigação precisa de ser aprofundada.")}<h4>Potencial de impacto</h4>${renderStructuredList(qualification.businessImpactHypotheses, "Ainda por validar.")}<h4>Perguntas para validar</h4>${renderStructuredList(qualification.discoveryQuestions)}<h4>Decisor provável</h4>${renderStructuredList(qualification.decisionMakerSignals, "Desconhecido; não bloqueia o Lead.")}<h4>Próximo passo</h4><p><a href="https://diagnostico-digital.uneed.pt/" target="_blank" rel="noopener noreferrer">Diagnóstico Digital Uneed</a></p><h4>Mensagem High Ticket</h4><blockquote>${escapeHtml(outreach.preparedMessage || "Ainda sem evidência suficiente para preparar uma mensagem.")}</blockquote></section>`; }
