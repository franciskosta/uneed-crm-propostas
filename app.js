...
function renderMissionResult(mission, compact = false) {
  ...
  return `<article class="mission-result ${compact ? "compact" : ""}">
    ...
    ${highTicket ? `<section class="mission-commercial-summary"><h4>Sinais operacionais confirmados</h4>${renderStructuredList(qualification.confirmedOperationalSignals, "Sem sinais suficientes.")}<h4>Hipóteses de oportunidade</h4>${renderStructuredList(qualification.opportunityHypotheses, "A investigação precisa de ser aprofundada.")}<h4>Perguntas para validar</h4>${renderStructuredList(qualification.discoveryQuestions)}<h4>Decisor sugerido</h4>${renderStructuredList(qualification.decisionMakerSignals, "Desconhecido; não bloqueia o Lead.")}<h4>Próximo passo</h4><p><a href="https://diagnostico-digital.uneed.pt/" target="_blank" rel="noopener noreferrer">Diagnóstico Digital Uneed</a></p><h4>Mensagem preparada</h4><blockquote>${escapeHtml(outreach.preparedMessage || "Ainda sem evidência suficiente para preparar uma mensagem.")}</blockquote></section>` : `
    <section class="mission-commercial-summary"><h4>Porque contactar</h4><p>${escapeHtml(qualification.whyContactThisLead || qualification.reasoningSummary || "A validar")}</p><h4>Principal oportunidade</h4><p>${escapeHtml(qualification.mainCommercialGap || structuredText(qualification.potentialNeeds?.[0]) || "A validar")}</p><h4>Solução UNEED</h4><p><strong>${escapeHtml(qualification.recommendedService || "A validar")}</strong>${qualification.serviceDetails?.priceFrom ? ` · desde ${escapeHtml(qualification.serviceDetails.priceFrom)}€ + IVA/mês · sem fidelização` : ""}</p><h4>Estratégia</h4><p>${escapeHtml(outreach.suggestedApproach || "A validar")}</p><h4>Mensagem</h4><blockquote>${escapeHtml(outreach.preparedMessage || "A preparar")}</blockquote><h4>Canais</h4><p>${escapeHtml(outreach.suggestedApproach?.split(". Ângulo:")[0] || "A validar")}</p></section>
    `}
    <h4>Lacunas confirmadas</h4>${renderStructuredList(qualification.confirmedGaps || qualification.problemsDetected, "Nenhuma lacuna confirmada com a evidência disponível.")}
    <h4>Oportunidades de melhoria</h4>${renderStructuredList(qualification.improvementOpportunities || qualification.potentialNeeds, "A validar comercialmente.")}
    <h4>Qualidade da comunicação digital</h4><p>${escapeHtml(qualification.communicationQuality ? `${structuredText(qualification.communicationQuality)} ` : "Ainda não avaliada.")}</p>
...
function reconcileLeadFactoryMissions() {
  ...
  for (const prospect of state.instagramProspects || []) {
    ...
    if (prospect.acquisitionStrategy === "high_ticket" && prospect.readiness === "ready_for_review" && prospect.commercialScore < Number(prospect.minimumScore || 0)) { prospect.readiness = "reject"; prospect.nextAction = `Rejeitado: High Ticket Fit ${prospect.commercialScore} abaixo do mínimo ${prospect.minimumScore}`; }
...
async function startInstagramResearch(prospectId, trigger) {
  ...
  const mission = await createCommercialMission({ type: "research_company", objective: `Investigar ${context.company.name}`, targetType: "company", targetId: context.company.id, input: { ...context, acquisitionStrategy: prospect.acquisitionStrategy, serviceId: prospect.serviceId }, trigger });
